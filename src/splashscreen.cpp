...
std::optional<Image> decodeJpegToRgba(const std::string& path)
{
    ...
    if (!fp)
    {
        std::fprintf(stderr, "decodeJpeg: failed to open '%s' (%s)\n", path.c_str(), std::strerror(errno));
        return std::nullopt;
    }
    ...
    if (setjmp(jerr.jump))
    {
        ...
        std::fclose(fp);
        std::fprintf(stderr, "decodeJpeg: decode failed '%s'\n", path.c_str());
        return std::nullopt;
    }
    ...
    {
        std::fprintf(stderr, "decodeJpeg: unsupported output format after conversion: %dx%d comps=%d\n", width, height, components);
...
std::optional<Image> decodePngToRgba(const std::string& path)
{
    ...
    if (!fp)
    {
        std::fprintf(stderr, "decodePng: failed to open '%s' (%s)\n", path.c_str(), std::strerror(errno));
        return std::nullopt;
    }
    ...
    {
        std::fprintf(stderr, "decodePng: invalid signature '%s'\n", path.c_str());
        std::fclose(fp);
        return std::nullopt;
    }
    ...
    if (setjmp(png_jmpbuf(pngPtr)))
    {
        ...
        std::fclose(fp);
        std::fprintf(stderr, "decodePng: decode failed '%s'\n", path.c_str());
        return std::nullopt;
    }
    ...
    // Standard libpng decode pattern: apply transforms to normalize into an
    // 8-bit RGB/RGBA output format, then read rows. (Authored for this project.)
    ...
    if (outBitDepth != 8)
    {
        std::fprintf(stderr, "decodePng: unsupported output format after transforms: colorType=%d bitDepth=%d channels=%d\n",
    ...
    if (outChannels == 4)
    {
        if (rowBytes != width * 4)
        {
            std::fprintf(stderr, "decodePng: unexpected rowBytes=%zu width=%u\n", static_cast<size_t>(rowBytes), width);
            png_destroy_read_struct(&pngPtr, &infoPtr, nullptr);
            std::fclose(fp);
            return std::nullopt;
        }
    ...
    {
        std::fprintf(stderr, "decodePng: unsupported output channels after transforms: colorType=%d bitDepth=%d channels=%d\n",
                    outColorType, outBitDepth, outChannels);
        png_destroy_read_struct(&pngPtr, &infoPtr, nullptr);
        std::fclose(fp);
        return std::nullopt;
...
ImageType detectImageTypeByMagic(const std::string& path)
{
    ...
    if (!fp)
    {
        std::fprintf(stderr, "detectImageType: could not open '%s'\n", path.c_str());
...
void onDisplaySize(void* userData, int w, int h)
{
    ...
    if (state->width != w || state->height != h)
    {
        state->width = w;
        state->height = h;
        std::fprintf(stderr, "display size: %dx%d\n", w, h);
    }
}
...
int main(int argc, char** argv)
{
    ...
    if (!ctx)
    {
        std::fprintf(stderr, "EssContextCreate failed\n");
        return 3;
    }
    ...
    {
        std::fprintf(stderr, "EssContextInit failed\n");
        ok = false;
    }

    if (ok && !EssContextGetDisplaySize(ctx, &display.width, &display.height))
    {
        std::fprintf(stderr, "EssContextGetDisplaySize failed\n");
        ok = false;
    }

    if (ok && !EssContextSetInitialWindowSize(ctx, display.width, display.height))
    {
        std::fprintf(stderr, "EssContextSetInitialWindowSize failed\n");
        ok = false;
    }

    if (ok && !EssContextStart(ctx))
    {
        std::fprintf(stderr, "EssContextStart failed\n");
        ok = false;
    }
    ...
    if (!opt->imagePath.empty())
    {
        ...
        if (!image)
        {
            std::fprintf(stderr, "Failed to decode image: %s (using fallback)\n", opt->imagePath.c_str());
        }
    }
    ...
    if (!ok)
    {
        ...
        std::fprintf(stderr, "Startup failed. Essos detail: %s\n", (detail ? detail : "(none)"));
    ...
    while (!gShouldQuit)
    {
        ...
        if (display.width <= 0 || display.height <= 0)
        {
            if (!warnedNoDisplaySize)
            {
                ...
                std::fprintf(stderr, "Display size not available yet (got %dx%d); waiting for settings update...\n",
                            display.width, display.height);
            }

            // default: eventLoopPeriodMs = 16 (60Hz)
            // This will be set only if the throttling is enabled (ESSOS_NO_EVENT_LOOP_THROTTLE is set)
        ...
        if (shouldDraw)
        {
            ...
            {
                firstFrame = false;
                std::fprintf(stderr, "RDKE_Splash first_frame_ms=%llu decode_ms=%llu gl_setup_ms=%llu img=%dx%d\n",
